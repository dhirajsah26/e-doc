...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>User Role Menu Management <span><i class="fa fa-angle-double-right"></i>
				${title}</span></h1>
	</section>
	...
	<section class="content">
		...
		<div class="box">
			...
			<div class="box-body">
				<form
					action="${baseUrl}/admin/package/template/${packageTemplateInfo.id}/edit"
					method="post" id="package-template">
					<c:set var="defaultDate"
						value="${operationDateSetting == 'np' ? helper.getCurrentNepaliDateOnly() : helper.getCurrentEnglishDateOnly()}"></c:set>

					<input type="hidden" value="${packageTemplateInfo.id}" name="id" id="package-id"/>

					<div class="form-group">
						<label>Package Name <span class="mandatory">*</span></label> <input
							type="text" class="form-control" name="packageTemplateName" value="${packageTemplateInfo.name}" required>
					</div>
					...
					<div class="form-group">
						<label>Remarks </label> 
						<textarea class="form-control" cols="6" name="remark" maxlength="500">${packageTemplateInfo.remarks}</textarea>
					</div>
					
					<div>
						<button type="button" class="btn btn-blue" onclick="submitForm(this)">Update</button>
...
<script type="text/javascript">
	...
	function populatepackagetemplates() {
		...
		var url = baseUrl + "/admin/package/template/populatepackagetemplatesforedit";
