...
@Controller
@RequestMapping(value = "doctor/qualification")
public class DoctorQualificationController {
    ...
    public String indexPage(Model model, HttpSession session, RedirectAttributes redirectAttributes) {
        ...
        if (SessionHelper.isAdminLoggedIn(session)) {
            ...
            List<DoctorQualificationDTO> doctorQualificationDTOList= doctorQualificationDao.findQualificationAndDoctorName();
        ...
        if (SessionHelper.isDoctorLoggedIn(session)) {
            ...
            List<DoctorQualificationDTO> doctorQualificationDTOList= doctorQualificationDao.findQualificationByDoctorID(SessionHelper.getLoggedInDoctorId(session).longValue());
            model.addAttribute("doctorQualificationDTOList",doctorQualificationDTOList);
            login="doctor";
        }
        ...
        return "doctor-qualification/index";
    }

    @RequestMapping(value = "/save", method = RequestMethod.POST)
    public ResponseEntity<Response> saveOrUpdatePersonalDetails(@ModelAttribute Qualification qualification, HttpSession session,
        ...
        try {
            ...
            qualification.setProcurementYear(Helper.parseDate(dateStr));
            qualification.setCreatedDate(new Date());

            if (doctorQualificationDao.save(qualification)) {
                success = true;
                message = "Doctor qualification successfully.";
    ...
    public ResponseEntity<Response> deleteSpecialization(@RequestParam Long id, Model model) {
        ...
        if (doctorQualificationDao.delete(id)) {
            success = true;
            message = "Doctor qualification deleted successfully.";
        } else {
